...
// --------------- Q1 ---------------
// Bananas is first because it is capitalized
console.log(simpleList.sort());

console.log(simpleList.map(s => s.toLowerCase()).sort());

// Backwards
console.log(simpleList.sort((s1, s2) => s1 < s2 ? 1 : -1)); 

// --------------- Q2 ---------------
console.log(simpleList.filter((s) => s > 'm'));

function searchList(list, query) {
    const method = (s) => s.toLowerCase().includes(query);
...
console.log(hikes.filter((hike) => hike.name.includes("Falls")));

// Tag list attribute doesn't exist on the objects
function searchHikes(query) {
    query = query.toLowerCase()
    return hikes.filter(
        (hike) => hike.name.toLowerCase().includes(query) || hike.description.toLowerCase().includes(query)
    );
}

console.log(searchHikes("short"));
console.log(searchHikes("river"));
